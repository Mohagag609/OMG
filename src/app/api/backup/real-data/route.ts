...
${users.map(user => `INSERT INTO users (id, name, email, password_hash, role, phone, address, is_active, last_login, created_at, updated_at) VALUES (${user.id}, '${user.name.replace(/'/g, "''")}', '${user.email}', '${user.password_hash}', '${user.role}', ${user.phone ? `'${user.phone}'` : 'NULL'}, ${user.address ? `'${user.address.replace(/'/g, "''")}'` : 'NULL'}, ${user.is_active}, ${user.last_login ? `'${user.last_login.toISOString()}'` : 'NULL'}, '${user.created_at.toISOString()}', '${user.updated_at.toISOString()}');`).join('\n')}

-- Insert real units data
${units.map(unit => `INSERT INTO units (id, code, name, type, area, price, status, floor_number, building_number, description, created_at, updated_at) VALUES (${unit.id}, '${unit.code}', '${unit.name.replace(/'/g, "''")}', '${unit.type}', ${unit.area || 'NULL'}, ${unit.price || 'NULL'}, '${unit.status}', ${unit.floor_number || 'NULL'}, ${unit.building_number ? `'${unit.building_number}'` : 'NULL'}, ${unit.description ? `'${unit.description.replace(/'/g, "''")}'` : 'NULL'}, '${unit.created_at.toISOString()}', '${unit.updated_at.toISOString()}');`).join('\n')}

-- Insert real customers data
${customers.map(customer => `INSERT INTO customers (id, name, phone, email, national_id, address, created_at, updated_at) VALUES (${customer.id}, '${customer.name.replace(/'/g, "''")}', ${customer.phone ? `'${customer.phone}'` : 'NULL'}, ${customer.email ? `'${customer.email}'` : 'NULL'}, ${customer.national_id ? `'${customer.national_id}'` : 'NULL'}, ${customer.address ? `'${customer.address.replace(/'/g, "''")}'` : 'NULL'}, '${customer.created_at.toISOString()}', '${customer.updated_at.toISOString()}');`).join('\n')}

-- Insert real brokers data
${brokers.map(broker => `INSERT INTO brokers (id, name, phone, email, commission_rate, status, created_at, updated_at) VALUES (${broker.id}, '${broker.name.replace(/'/g, "''")}', ${broker.phone ? `'${broker.phone}'` : 'NULL'}, ${broker.email ? `'${broker.email}'` : 'NULL'}, ${broker.commission_rate || 0}, '${broker.status}', '${broker.created_at.toISOString()}', '${broker.updated_at.toISOString()}');`).join('\n')}

-- Insert real contracts data
${contracts.map(contract => `INSERT INTO contracts (id, unit_id, customer_id, broker_id, contract_number, total_amount, down_payment, remaining_amount, contract_date, status, notes, created_at, updated_at) VALUES (${contract.id}, ${contract.unit_id || 'NULL'}, ${contract.customer_id || 'NULL'}, ${contract.broker_id || 'NULL'}, '${contract.contract_number}', ${contract.total_amount}, ${contract.down_payment || 0}, ${contract.remaining_amount}, '${contract.contract_date}', '${contract.status}', ${contract.notes ? `'${contract.notes.replace(/'/g, "''")}'` : 'NULL'}, '${contract.created_at.toISOString()}', '${contract.updated_at.toISOString()}');`).join('\n')}

-- Insert real installments data
${installments.map(installment => `INSERT INTO installments (id, contract_id, installment_number, amount, due_date, paid_date, status, payment_method, notes, created_at, updated_at) VALUES (${installment.id}, ${installment.contract_id}, ${installment.installment_number}, ${installment.amount}, '${installment.due_date}', ${installment.paid_date ? `'${installment.paid_date}'` : 'NULL'}, '${installment.status}', ${installment.payment_method ? `'${installment.payment_method}'` : 'NULL'}, ${installment.notes ? `'${installment.notes.replace(/'/g, "''")}'` : 'NULL'}, '${installment.created_at.toISOString()}', '${installment.updated_at.toISOString()}');`).join('\n')}

-- Insert real safes data
${safes.map(safe => `INSERT INTO safes (id, name, balance, currency, description, created_at, updated_at) VALUES (${safe.id}, '${safe.name.replace(/'/g, "''")}', ${safe.balance || 0}, '${safe.currency}', ${safe.description ? `'${safe.description.replace(/'/g, "''")}'` : 'NULL'}, '${safe.created_at.toISOString()}', '${safe.updated_at.toISOString()}');`).join('\n')}

-- Insert real partners data
${partners.map(partner => `INSERT INTO partners (id, name, phone, email, percentage, balance, address, created_at, updated_at) VALUES (${partner.id}, '${partner.name.replace(/'/g, "''")}', ${partner.phone ? `'${partner.phone}'` : 'NULL'}, ${partner.email ? `'${partner.email}'` : 'NULL'}, ${partner.percentage || 0}, ${partner.balance || 0}, ${partner.address ? `'${partner.address.replace(/'/g, "''")}'` : 'NULL'}, '${partner.created_at.toISOString()}', '${partner.updated_at.toISOString()}');`).join('\n')}

-- Insert real vouchers data
${vouchers.map(voucher => `INSERT INTO vouchers (id, voucher_number, type, amount, description, date, safe_id, created_at, updated_at) VALUES (${voucher.id}, '${voucher.voucher_number}', '${voucher.type}', ${voucher.amount}, ${voucher.description ? `'${voucher.description.replace(/'/g, "''")}'` : 'NULL'}, '${voucher.date}', ${voucher.safe_id || 'NULL'}, '${voucher.created_at.toISOString()}', '${voucher.updated_at.toISOString()}');`).join('\n')}

-- Insert real transfers data
${transfers.map(transfer => `INSERT INTO transfers (id, from_safe_id, to_safe_id, amount, description, date, created_at, updated_at) VALUES (${transfer.id}, ${transfer.from_safe_id || 'NULL'}, ${transfer.to_safe_id || 'NULL'}, ${transfer.amount}, ${transfer.description ? `'${transfer.description.replace(/'/g, "''")}'` : 'NULL'}, '${transfer.date}', '${transfer.created_at.toISOString()}', '${transfer.updated_at.toISOString()}');`).join('\n')}

-- Insert real unit_partners data
${unitPartners.map(up => `INSERT INTO unit_partners (id, unit_id, partner_id, percentage, created_at, updated_at) VALUES (${up.id}, ${up.unit_id}, ${up.partner_id}, ${up.percentage}, '${up.created_at.toISOString()}', '${up.updated_at.toISOString()}');`).join('\n')}

-- Insert real broker_dues data
${brokerDues.map(bd => `INSERT INTO broker_dues (id, broker_id, contract_id, amount, due_date, paid_date, status, created_at, updated_at) VALUES (${bd.id}, ${bd.broker_id}, ${bd.contract_id}, ${bd.amount}, '${bd.due_date}', ${bd.paid_date ? `'${bd.paid_date}'` : 'NULL'}, '${bd.status}', '${bd.created_at.toISOString()}', '${bd.updated_at.toISOString()}');`).join('\n')}

-- Insert real partner_debts data
${partnerDebts.map(pd => `INSERT INTO partner_debts (id, partner_id, amount, due_date, paid_date, status, description, created_at, updated_at) VALUES (${pd.id}, ${pd.partner_id}, ${pd.amount}, '${pd.due_date}', ${pd.paid_date ? `'${pd.paid_date}'` : 'NULL'}, '${pd.status}', ${pd.description ? `'${pd.description.replace(/'/g, "''")}'` : 'NULL'}, '${pd.created_at.toISOString()}', '${pd.updated_at.toISOString()}');`).join('\n')}

-- Insert real partner_groups data
${partnerGroups.map(pg => `INSERT INTO partner_groups (id, name, description, created_at, updated_at) VALUES (${pg.id}, '${pg.name.replace(/'/g, "''")}', ${pg.description ? `'${pg.description.replace(/'/g, "''")}'` : 'NULL'}, '${pg.created_at.toISOString()}', '${pg.updated_at.toISOString()}');`).join('\n')}

-- Insert real audit_logs data
${auditLogs.map(log => `INSERT INTO audit_logs (id, user_id, action, table_name, record_id, old_values, new_values, ip_address, user_agent, created_at) VALUES (${log.id}, ${log.user_id || 'NULL'}, '${log.action}', ${log.table_name ? `'${log.table_name}'` : 'NULL'}, ${log.record_id || 'NULL'}, ${log.old_values ? `'${JSON.stringify(log.old_values).replace(/'/g, "''")}'` : 'NULL'}, ${log.new_values ? `'${JSON.stringify(log.new_values).replace(/'/g, "''")}'` : 'NULL'}, ${log.ip_address ? `'${log.ip_address}'` : 'NULL'}, ${log.user_agent ? `'${log.user_agent.replace(/'/g, "''")}'` : 'NULL'}, '${log.created_at.toISOString()}');`).join('\n')}
